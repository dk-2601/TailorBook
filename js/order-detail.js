(function () {
  ...
  function render() {
    ...
    panel.innerHTML =
      "<div class='highlight-card'><p class='label'>Total</p><p class='value'>₹ " + total.toFixed(0) + "</p></div>" +
      "<div class='highlight-card'><p class='label'>Material Cost</p><p class='value'>₹ " + materialCost.toFixed(0) + "</p></div>" +
      "<div class='highlight-card'><p class='label'>Paid Amount</p><p class='value'>₹ " + paid.toFixed(0) + "</p></div>" +
      "<div class='highlight-card'><p class='label'>Balance</p><p class='value'>₹ " + balance.toFixed(0) + "</p></div>" +
      "<div class='highlight-card'><p class='label'>Margin</p><p class='value'>₹ " + margin.toFixed(0) + "</p></div>" +
    ...
    head.innerHTML =
      ...
      "</button>" +
      (shouldShowNotify(order.status) ?
      "<button type='button' id='orderNotifyBtn' class='mini-btn wa-btn icon-only' aria-label='Message customer' title='Message customer'>" +
      "<span class='btn-icon' aria-hidden='true'>" + iconSvg("whatsapp") + "</span>" +
      "</button>" : "") +
    ...
    const notifyBtn = document.getElementById("orderNotifyBtn");
    if (notifyBtn) {
    ...
    if (editMode) {
      ...
      function saveOrderChanges() {
        ...
        const shouldAuto = prevStatus !== nextStatus && shouldShowNotify(nextStatus);
        ...
        render();
        if (shouldAuto) {
          const latest = TailorStorage.getOrderById ? TailorStorage.getOrderById(order.id) : null;
          openNotifyModal(latest || Object.assign({}, order, updated));
        }
      }
  ...
  function shouldShowNotify(status) {
  ...
  function buildStatusMessage(order, totals) {
    ...
    if (s === "ready") {
      lines.push("Your order #" + orderNo + " is ready.");
      lines.push("Please collect at your convenience.");
      if (totals && totals.balance > 0) {
        lines.push("");
        lines.push("Balance: " + formatMoneyMsg(totals.balance));
      }
    } else if (s === "delivered") {
      lines.push("Your order #" + orderNo + " has been delivered.");
      if (totals && totals.balance > 0) {
        lines.push("");
        lines.push("Balance: " + formatMoneyMsg(totals.balance));
      }
    } else {
      lines.push("Update on your order #" + orderNo + ".");
    }
  ...
  function getOrderTotalsForMessage(order) {
    const paid = resolvePaid(order);
  ...
  function openNotifyModal(order) {
    ...
    const waPhone = normalizeWaPhone(order.customerPhone);

    const modal = document.createElement("div");
    modal.className = "modal center";

    const card = document.createElement("div");
    card.className = "modal-card msg-modal";

    const head = document.createElement("div");
    head.className = "modal-head";

    const title = document.createElement("h3");
    title.textContent = "Send WhatsApp";

    const closeBtn = document.createElement("button");
    closeBtn.type = "button";
    closeBtn.className = "icon-btn";
    closeBtn.setAttribute("aria-label", "Close");
    closeBtn.innerHTML = "<span class='btn-icon' aria-hidden='true'>" + iconSvg("close") + "</span>";
    ...
    muted.innerHTML = "To: <strong>" + escapeHtml(order.customerPhone || "-") + "</strong>";

    const totals = getOrderTotalsForMessage(order);

    const ta = document.createElement("textarea");
    ta.rows = 10;
    ta.value = buildStatusMessage(order, totals);

    const actions = document.createElement("div");
    actions.className = "save-bar";

    const sendBtn = document.createElement("button");
    sendBtn.type = "button";
    sendBtn.className = "wa-btn";
    sendBtn.textContent = "Open WhatsApp";
  ...
  function iconSvg(name) {
    const icons = {
      ...
      whatsapp:
        "<svg viewBox='0 0 24 24' role='img'><path d='M21 11.5a8.5 8.5 0 0 1-12.7 7.4L3 20l1.2-4.2A8.5 8.5 0 1 1 21 11.5Z'></path><path d='M8.5 10.5c.5 2 2.5 4 4.5 4.5'></path><path d='M13 15l2-1'></path></svg>"
    };
    return icons[name] || "";
  }
